...
if __name__ == '__main__':
    ...
    for core_index, core in enumerate(n_core_list):
        ...
        # get RT allocation
        rt_alloc_prop = TALLOC.get_rt_task_assignemnt(tc_conf, PARAMS.EXPERIMENT_NAME_PROP, worst_fit=True)

        rt_alloc_ref = TALLOC.get_rt_task_assignemnt(tc_conf, PARAMS.EXPERIMENT_NAME_REF, worst_fit=True)
        rt_alloc_ref = np.concatenate((rt_alloc_ref, np.zeros((tc_conf.n_rt_task, 1))),axis=1)  # append the zero allocation to the Mth core



        # some error checking
        if rt_alloc_prop is None or rt_alloc_ref is None:
        ...
        _, se_opt_period_list_prop, _, x = TALLOC.allocate_security_task(tc_conf_prop, rt_alloc_prop, True)
        _, se_opt_period_list_ref, _ = TALLOC.find_se_task_param_reference_scheme(tc_conf_ref)
        ...
        for i in range(0, PARAMS.N_SIM_SAMPLES):
            ...
            dt_both_prop, dt_any_prop = SIM.simulate_schedule(core, tc_conf_prop, rt_alloc_prop, x, sim_duration, attack_time, se_task_attack_detector_list, PARAMS.EXPERIMENT_NAME_PROP)
            dt_both_ref, dt_any_ref = SIM.simulate_schedule(core, tc_conf_prop, rt_alloc_ref, x, sim_duration, attack_time, se_task_attack_detector_list,
